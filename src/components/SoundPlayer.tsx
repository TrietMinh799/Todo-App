...
import { SoundType, sounds, useSoundPlayer } from '../services/soundService';
...
export const SoundPlayer: React.FC = () => {
  ...
  const currentSoundName = sounds.find(s => s.id === currentSound)?.name || 'No Sound';
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className={`absolute top-full right-0 mt-2 w-48 rounded-lg shadow-lg overflow-hidden z-10 ${
